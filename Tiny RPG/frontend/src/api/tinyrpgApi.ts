...
const ACCESS_TOKEN_KEY = 'tinyrpg.access_token'
export const AUTH_EXPIRED_EVENT = 'tinyrpg:auth-expired'

export type UserResponse = { id: number; email: string; display_name: string; created_at: string }
...
async function request<T>(path: string, options: RequestInit = {}, requiresAuthentication = false): Promise<T> {
